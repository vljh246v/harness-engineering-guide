...
├── service/          # @Service — 비즈니스 규칙, 유효성 검사
├── repository/       # @Repository — 데이터 저장/조회
└── domain/           # data class — 순수 모델 (의존성 없음)
```
...
- `controller`에서 `repository`를 직접 주입하지 않는다
- `domain`은 Spring 어노테이션을 포함하지 않는다
- 비즈니스 로직은 `service`에만 위치한다
- 유효성 검사 (`require`, `check`)는 `service` 레이어에서만
